import {
	...
	onSessionWorkspaceUpdated,
	respondClarify,
	type ChatRunTransport,
	type RunEvent,
...
import {
	...
	type HermesMessage,
	type SessionSummary,
	type WorkspaceRunChangeFileDetail,
...
type ChatAgentId = "hermes" | "claude" | "codex";
...
function moaReferenceLabel(evt: RunEvent): string {
	const label =
		typeof evt.label === "string" && evt.label.trim()
			? evt.label.trim()
			: "reference";
	const index = Number.isFinite(Number(evt.index))
		? Number(evt.index)
		: undefined;
	const count = Number.isFinite(Number(evt.count))
		? Number(evt.count)
		: undefined;
	return index != null && count != null ? `${index}/${count} ${label}` : label;
}
...
function subagentStatus(
	value: unknown,
	fallback: SubagentStreamStatus = "running",
...
function subagentEntryId(eventName: string, evt: RunEvent): string {
	...
	if (Number.isFinite(backgroundSequence))
		return `${eventName}:${backgroundSequence}`;
...
export function reduceSubagentStream(
	...
	const subagentId = String(
		(evt as any).subagent_id ||
			evt.delegation_id ||
			`${(evt as any).task_index ?? 0}`,
	);
	const timestamp = subagentEventTimestamp(
		evt.timestamp || (evt as any).updated_at,
	);
	const backgroundSequence = Number((evt as any).background_seq);
	if (
		current &&
	...
	const isTerminal =
		...
		(eventName === "delegation.updated" &&
			["completed", "failed", "error", "cancelled", "interrupted"].includes(
				String((evt as any).status || "").toLowerCase(),
			));
	...
	if (eventName === "subagent.start") {
		if (
			!entries.some(
				(entry) => entry.kind === "status" && entry.status === "started",
			)
		) {
			entries.push({
				id: entryId,
		...
		if (previous?.kind === "status" && previous.status === "running")
	...
	if (isTerminal) {
		...
		if (previous?.kind === "status" && previous.status !== "started")
			entries[entries.length - 1] = terminalEntry;
	...
	return {
		sessionId,
		subagentId,
		taskIndex: Number.isFinite(incomingTaskIndex)
			? incomingTaskIndex
			: current?.taskIndex || 0,
		taskCount:
			Number.isFinite(incomingTaskCount) && incomingTaskCount > 0
				? incomingTaskCount
				: current?.taskCount || 1,
		...
		startedAt:
			current?.startedAt ||
			subagentEventTimestamp((evt as any).started_at || timestamp),
		updatedAt: timestamp,
		completedAt: isTerminal
			? subagentEventTimestamp((evt as any).completed_at || timestamp)
			: current?.completedAt,
		...
		lastBackgroundSeq: Number.isFinite(backgroundSequence)
			? backgroundSequence
			: current?.lastBackgroundSeq,
...
export function formatMessageWithReference(
...
export function attachWorkspaceChangesToExactTurns(
	...
	for (const change of changes) {
		...
		const target = assistantMessageId
			? assistantById.get(assistantMessageId)
			: undefined;
			...
			if (attachment?.type.startsWith("image/")) {
				blocks.push({
					...
					...(attachment.context?.trim()
						? { context: attachment.context.trim() }
						: {}),
				});
			} else {
				// Other files
...
function isBackgroundDelegateToolPayload(
	toolName: unknown,
	payload: unknown,
): boolean {
...
function backgroundDelegateAnchorCallId(
	baseId: string,
	taskIndex: number,
): string {
...
function parsePersistedMoaToolPayload(
	toolName: string | undefined,
	value: unknown,
): { preview?: string; result?: unknown } | null {
	if (toolName !== "moa_reference" && toolName !== "moa_aggregating")
		return null;
	const payload =
		typeof value === "string"
			? (() => {
...
function isPersistedMoaToolDisplay(msg: HermesMessage): boolean {
	return (
		(msg.role === "moa" || msg.display_role === "tool") &&
...
function selectResumedInFlightAssistant(
	messages: Message[],
...
function resolveResumedAssistantState(
	...
	const activeAssistant = options.previousActiveAssistantMessageId
		? messages.find(
				(m) =>
					m.role === "assistant" &&
					m.id === options.previousActiveAssistantMessageId,
			) || null
		: null;
	const selectedActiveAssistant =
		activeAssistant ||
	...
	const reasoningAssistant = options.previousReasoningAssistantMessageId
		? messages.find(
				(m) =>
					m.role === "assistant" &&
					m.id === options.previousReasoningAssistantMessageId,
			) || null
		: null;
	const selectedReasoningAssistant =
		reasoningAssistant ||
		(selectedActiveAssistant?.reasoning ? selectedActiveAssistant : null);
	const selectedRunMarker =
		readRunMarker(selectedActiveAssistant) ?? options.activeRunMarker ?? null;
	return {
...
function mapHermesMessages(msgs: HermesMessage[]): Message[] {
	...
	const filteredMsgs = msgs.filter((m) => {
		if (m.role === "assistant") {
			return (
				...
				runtimePayloadText((m as any).content).trim() !== ""
			);
		}
		return true;
	});
	...
	for (const msg of filteredMsgs) {
		...
		if (msg.role === "tool" || isPersistedMoaToolDisplay(msg)) {
			...
			const toolArgs = toolArgsMap.has(tcId)
				? toolArgsMap.get(tcId)
				: undefined;
			const moaPayload = parsePersistedMoaToolPayload(
				toolName,
				(msg as any).content,
			);
			const backgroundDelegate = isBackgroundDelegateToolPayload(
				toolName,
				(msg as any).content,
			);
			const delegatePayload =
				toolName === "delegate_task"
					? runtimeObjectPayload(msg.display_content) ||
						runtimeObjectPayload((msg as any).content)
					: null;
			...
			if (delegatePayload?.runtime === "hermes") {
				const persistedTasks = Array.isArray(delegatePayload.tasks)
					? (delegatePayload.tasks.filter(
							(task) => task && typeof task === "object",
						) as Array<Record<string, unknown>>)
					: [delegatePayload];
				const restorableTasks = persistedTasks.filter((task) =>
					String(task.subagent_id || "").trim(),
				);
				if (restorableTasks.length > 0) {
					for (const [index, task] of restorableTasks.entries()) {
						...
						const taskCount = Math.max(
							1,
							Number(task.task_count ?? restorableTasks.length) || 1,
						);
						...
						result.push({
							...
							toolArgs,
							toolPreview:
								`${label}${summary ? ` · ${summary}` : goal ? ` · ${goal}` : ""}`.slice(
									0,
									220,
								),
							toolResult: task,
			...
			if (backgroundDelegate && delegatePayload) {
				...
				for (const task of backgroundDelegateTaskDescriptors(
					delegatePayload,
					toolArgs,
				)) {
					...
					result.push({
						...
						toolArgs,
						toolPreview: `${label}${task.goal ? ` · ${task.goal}` : ""}`.slice(
							0,
							220,
						),
						toolResult: {
			...
			result.push({
				...
				toolPreview:
					typeof preview === "string"
						...
						: undefined,
				toolResult: moaPayload
					? runtimeToolPayloadOrUndefined(moaPayload.result)
...
function sessionActivitySeconds(s: SessionSummary): number {
	return Math.max(s.started_at || 0, s.ended_at || 0, s.last_active || 0);
}
...
function mapHermesSession(s: SessionSummary): Session {
	...
	const isCodingAgentSession =
		s.source === "coding_agent" || Boolean(codingAgentId);
	...
	return {
		...
		forkPointMessageId:
			(s as any).fork_point_message_id != null
				? String((s as any).fork_point_message_id)
				: null,
		...
		lastActiveAt:
			s.last_active == null ? undefined : Math.round(s.last_active * 1000),
...
function runtimeStoragePrefix(): string {
	return activeRuntimeMode === "global_agent"
		? `${STORAGE_KEY_PREFIX}global_agent_`
		: STORAGE_KEY_PREFIX;
}

function storageKey(): string {
	return runtimeStoragePrefix() + getProfileName();
}
function legacyStorageKey(): string | null {
	return activeRuntimeMode === "default" && getProfileName() === "default"
		? LEGACY_STORAGE_KEY
		: null;
}

function isCodingAgentLikeSession(
	session?: Pick<Session, "source" | "agent" | "codingAgentId"> | null,
): boolean {
	return (
...
function shouldPreserveRuntimeApiMode(session?: Session | null): boolean {
	return (
		isCodingAgentLikeSession(session) && session?.codingAgentMode !== "global"
	);
}
...
export const useChatStore = defineStore("chat", () => {
	...
	const storedSessionProfileFilter = getItemBestEffort(
		SESSION_PROFILE_FILTER_STORAGE_KEY,
	)?.trim();
	...
	function setSessionProfileFilter(profile: string | null) {
		...
		sessionProfileFilter.value =
			normalized && normalized !== "__all__" ? normalized : null;
	...
	function validateSessionProfileFilter(profileNames: string[]) {
		...
		if (!current || profileNames.length === 0 || profileNames.includes(current))
			return;
	...
	async function fetchRuntimeSessions(
		...
		if (runtimeMode.value === "global_agent")
			return fetchSessions("global_agent", undefined, scopedProfile);
		...
		for (const session of [...localSessions, ...globalSessions])
			byId.set(session.id, session);
		return [...byId.values()].sort(
			(a, b) => sessionActivitySeconds(b) - sessionActivitySeconds(a),
		);
	}
	...
	function setAbortState(
		sessionId: string | null | undefined,
		state: AbortState | null,
	) {
		if (!sessionId) return;
	...
	const workspaceRunChangesBySession = ref<
		Map<string, Map<string, WorkspaceRunChangeSummary>>
	>(new Map());
	...
	function pruneCompletedUnreadSessions(existingIds: Set<string>) {
		const next = new Set(
			[...completedUnreadSessions.value].filter((id) => existingIds.has(id)),
		);
		if (next.size !== completedUnreadSessions.value.size)
			completedUnreadSessions.value = next;
	}
	...
	function attachToolChangesToMessages(sessionId: string) {
		...
		const existingById = new Map(
			target.messages
				.filter((message) =>
					message.id.startsWith(WORKSPACE_RUN_CHANGE_MESSAGE_PREFIX),
				)
				.map((message) => [message.id, message]),
		);
		target.messages = target.messages.filter(
			(message) => !message.id.startsWith(WORKSPACE_RUN_CHANGE_MESSAGE_PREFIX),
		);
		for (const message of target.messages) {
			if (message.role === "tool" && message.toolCallId) {
				const change = changes?.get(message.toolCallId);
				message.toolChange = String(change?.assistant_message_id || "").trim()
					? change
					: undefined;
			}
		}
		...
		const runChanges = [...changes.values()].filter(
			(change) => change?.source === "run",
		);
		const unresolvedAttributedChanges = attachWorkspaceChangesToExactTurns(
			target.messages,
			runChanges,
		);
		insertWorkspaceRunChangeMessages(
			target,
			unresolvedAttributedChanges,
			existingById,
		);
	}
	...
	function workspaceRunChangeTimestamp(
		change: WorkspaceRunChangeSummary,
	): number {
		const seconds = Number(
			change.finished_at || change.created_at || change.started_at || 0,
		);
	...
	function insertWorkspaceRunChangeMessages(
		...
		const sortedChanges = changes
			...
			.sort((a, b) => {
				const timeDelta =
					workspaceRunChangeTimestamp(b) - workspaceRunChangeTimestamp(a);
				return timeDelta !== 0
					? timeDelta
					: b.change_id.localeCompare(a.change_id);
			});
		...
		for (const change of sortedChanges) {
			...
			const insertAfter = findWorkspaceRunChangeAnchorIndex(
				target.messages,
				timestamp,
			);
	...
	function findWorkspaceRunChangeAnchorIndex(
		messages: Message[],
		timestamp: number,
	): number {
		for (let i = messages.length - 1; i >= 0; i -= 1) {
			...
			if (message.role === "assistant" && message.timestamp <= timestamp + 1000)
				return i;
		}
		for (let i = messages.length - 1; i >= 0; i -= 1) {
			if (!messages[i].id.startsWith(WORKSPACE_RUN_CHANGE_MESSAGE_PREFIX))
				return i;
		}
		return messages.length - 1;
	}

	function setWorkspaceRunChanges(
		sessionId: string,
		changes: WorkspaceRunChangeSummary[],
	) {
	...
	function upsertWorkspaceRunChange(
		sessionId: string,
		change: WorkspaceRunChangeSummary | null | undefined,
	) {
	...
	function handleTerminalWorkspaceRunChange(
		...
		const change = evt?.workspace_run_change as
			| WorkspaceRunChangeSummary
			| undefined;
		const target = sessions.value.find((session) => session.id === sessionId);
		if (target)
	...
	function restoreWorkspaceRunChangeMessages(sessionId: string) {
		attachToolChangesToMessages(sessionId);
		if (
			workspaceRunChangesBySession.value.has(sessionId) ||
			workspaceRunChangeLoadRequests.has(sessionId)
		)
			return;
		workspaceRunChangeLoadRequests.add(sessionId);
		void loadWorkspaceRunChangesForSession(sessionId)
			.catch((err) =>
				console.warn("Failed to load workspace run changes:", err),
			)
			.finally(() => {
	...
	async function loadWorkspaceRunChangeFile(
		sessionId: string,
		toolCallId: string,
		fileId: number,
	...
	async function refreshSessionListOnly(
		...
		try {
			const list = await fetchRuntimeSessions(
				profile ?? sessionProfileFilter.value,
			);
			...
			for (const fresh of incoming) {
				...
				if (existing) {
					...
					if (fresh.messageTotal != null) {
						existing.messageTotal = Math.max(
							fresh.messageTotal,
							existing.loadedMessageCount || 0,
						);
					}
					next.push(existing);
				} else {
	...
	async function refreshActiveSession(): Promise<boolean> {
		...
		try {
			...
			const limit = Math.min(
				Math.max(
					target.loadedMessageCount || LIVE_CHAT_MESSAGE_PAGE_SIZE,
			...
			target.parentSessionId =
				detail.session.parent_session_id || target.parentSessionId || null;
			target.forkPointMessageId =
				(detail.session as any).fork_point_message_id != null
					? String((detail.session as any).fork_point_message_id)
					: target.forkPointMessageId || null;
			target.parentTitle =
				detail.session.parent_title || target.parentTitle || null;
			target.parentLastMessage =
				detail.session.parent_last_message || target.parentLastMessage || null;
			target.parentLastMessageRole =
				detail.session.parent_last_message_role ||
				target.parentLastMessageRole ||
				null;
	...
	function createSession(
		...
		const source =
			runtimeMode.value === "global_agent"
				? "global_agent"
				: options.source || "cli";
		const codingAgentId =
			options.codingAgentId || agentToCodingAgentId(options.agent);
		const codingAgentMode = codingAgentId
			? options.codingAgentMode || "scoped"
			: undefined;
	...
	async function switchSession(sessionId: string, focusId?: string | null) {
			...
			await new Promise<void>((resolve, reject) => {
				...
				resumeSession(
					...
					(data) => {
						...
						target.parentSessionId =
							(data as any).parentSessionId || target.parentSessionId || null;
						target.forkPointMessageId =
							(data as any).forkPointMessageId != null
								? String((data as any).forkPointMessageId)
								: target.forkPointMessageId || null;
						target.parentTitle =
							(data as any).parentTitle || target.parentTitle || null;
						target.parentLastMessage =
							(data as any).parentLastMessage ||
							target.parentLastMessage ||
							null;
						target.parentLastMessageRole =
							(data as any).parentLastMessageRole ||
							target.parentLastMessageRole ||
							null;
						...
						if (data.events?.length) {
							for (const evt of data.events) {
								...
								if (e.event === "compression.started") {
									...
									if (
										isBackgroundDelegateToolPayload(
											startedToolName,
											(e as any).arguments,
										) ||
										(isEkkoAgentSession(sessionId) &&
											startedToolName === "delegate_task")
									)
										continue;
									...
									if (existingTool) {
										updateMessage(sessionId, existingTool.id, {
											...
											toolArgs: hasRuntimeToolPayload((e as any).arguments)
												? (e as any).arguments
										...
										addMessage(sessionId, {
											...
											toolArgs: runtimeToolPayloadOrUndefined(
												(e as any).arguments,
											),
											toolStatus: "running",
										});
									...
									const toolName =
										e.tool || e.name || toolMsgs[toolMsgs.length - 1]?.toolName;
									if (isBackgroundDelegateToolPayload(toolName, output)) {
										target.messages = target.messages.filter(
											(message) => !toolMsgs.includes(message),
										);
									...
									e.event === "moa.reference" ||
									e.event === "moa.aggregating"
								) {
									handleMoaEvent(sessionId, e as RunEvent);
								} else if (
									String(e.event || "").startsWith("subagent.") ||
									e.event === "delegation.updated"
								) {
		...
		if (
			...
			resumeServerWorkingRun(
				sessionId,
				backgroundPendingOnResume > 0,
	...
	async function loadOlderMessages(
		...
		const limit = Math.min(
			LIVE_CHAT_MESSAGE_PAGE_SIZE,
			LIVE_CHAT_MAX_LOADED_MESSAGES - offset,
		);
	...
	function newChat(
		...
		const codingAgentId =
			options.codingAgentId || agentToCodingAgentId(options.agent);
		const isGlobalCodingAgent =
			Boolean(codingAgentId) && options.codingAgentMode === "global";
		const session = createSession({
			profile: options.profile,
			model: isGlobalCodingAgent
				? undefined
				: options.model || appStore.selectedModel || undefined,
			provider: isGlobalCodingAgent
				? ""
				: options.provider || appStore.selectedProvider || "",
	...
	async function switchSessionModel(
		...
		const activeTarget =
			activeSession.value?.id === targetId ? activeSession.value : null;
		const previousProvider = String(
			target?.provider ?? activeTarget?.provider ?? "",
		);
		const nextProvider = provider || "";
		const shouldClearRuntimeCredentials =
			previousProvider !== nextProvider &&
			(isCodingAgentLikeSession(target) ||
				isCodingAgentLikeSession(activeTarget));
		const preservedApiMode =
			apiMode ||
			(previousProvider === nextProvider
				? (shouldPreserveRuntimeApiMode(target)
						? target?.apiMode
						: undefined) ||
					(shouldPreserveRuntimeApiMode(activeTarget)
						? activeTarget?.apiMode
						: undefined)
				: undefined);
		const isLocalOnly =
			target?.isLocalOnly === true || activeTarget?.isLocalOnly === true;
		if (!isLocalOnly) {
		...
		if (target) {
			...
			if (shouldClearRuntimeCredentials)
				clearCodingAgentRuntimeCredentials(target);
		}
		if (activeTarget) {
			...
			if (shouldClearRuntimeCredentials)
				clearCodingAgentRuntimeCredentials(activeTarget);
		}
		return true;
	}
	...
	// Stub: always returns false after ekko-agent removal.
	// Callers remain in this file and would need separate cleanup.
	function isEkkoAgentSession(_sessionId: string): boolean {
		return false;
	}
	...
	function addMessageInTimelineOrder(sessionId: string, msg: Message) {
		...
		if (!session) return;
		const insertAt = session.messages.findIndex(
	...
	function addHermesBackgroundDelegateAnchors(
		...
		if (!payload || payload.mode !== "background") return;
		...
		for (const task of backgroundDelegateTaskDescriptors(payload, toolArgs)) {
			const anchorCallId = backgroundDelegateAnchorCallId(
				baseId,
				task.taskIndex,
			);
			if (messages.some((message) => message.toolCallId === anchorCallId))
				continue;
			const label = `${task.taskIndex + 1}/${task.taskCount}`;
			addMessage(sessionId, {
				id: uid(),
	...
	function findHermesBackgroundDelegateAnchor(
		messages: Message[],
		evt: RunEvent,
	): Message | undefined {
		...
		const candidates = messages.filter(
			(message) =>
				message.role === "tool" &&
				message.toolCallId?.startsWith(
					HERMES_BACKGROUND_DELEGATE_ANCHOR_PREFIX,
				) &&
		...
		return (
			candidates.find((message) => {
				...
				return (
					Number(payload?.task_index ?? 0) === taskIndex &&
					(!goal ||
						!String(payload?.goal || "").trim() ||
						String(payload?.goal || "").trim() === goal)
				);
			}) ||
			candidates.find(
				(message) =>
					Number(runtimeObjectPayload(message.toolResult)?.task_index ?? 0) ===
					taskIndex,
			)
		);
	}
	...
	function settleRunningTools(sessionId: string, status: "done" | "error") {
		...
		msgs.forEach((m, i) => {
			if (
				...
				!m.toolCallId?.startsWith("subagent:")
			) {
	...
	function handleSubagentEvent(sessionId: string, evt: RunEvent) {
		...
		if (eventName === "delegation.updated") {
			...
			const sessionStreams = [...subagentStreams.value.values()].filter(
				(stream) =>
					stream.sessionId === sessionId && stream.status === "running",
			);
			const exactMatch = sessionStreams.find(
				(stream) => stream.subagentId === delegationId,
			);
		...
		if (eventName === "subagent.start") {
			...
			eventName === "subagent.text" ||
			eventName === "subagent.thinking"
		) {
		...
		const toolStatus =
			nextStream.status === "running"
				? "running"
				: nextStream.status === "completed"
					? "done"
					: "error";
	...
	function restorePersistedSubagentStreams(sessionId: string) {
		for (const message of getSessionMsgs(sessionId)) {
			if (
				message.role !== "tool" ||
				!message.toolCallId?.startsWith("subagent:")
			)
				continue;
			const subagentId = message.toolCallId.slice("subagent:".length).trim();
			if (
				!subagentId ||
				subagentStreams.value.has(`${sessionId}:${subagentId}`)
			)
				continue;
			const payload = runtimeObjectPayload(message.toolResult);
			if (!payload) continue;
			const status = subagentStatus(payload.status);
			const restoredOutput = String(
				payload.output || payload.output_tail || payload.summary || "",
			).trim();
			handleSubagentEvent(sessionId, {
				...payload,
	...
	function settleInterruptedSubagents(sessionId: string) {
		const runningStreams = [...subagentStreams.value.values()].filter(
			(stream) => stream.sessionId === sessionId && stream.status === "running",
		);
	...
	function handleMoaEvent(sessionId: string, evt: RunEvent) {
		const eventName = String(evt.event || "");
		if (eventName !== "moa.reference" && eventName !== "moa.aggregating")
			return;
		...
		const aggregator =
			typeof evt.aggregator === "string" && evt.aggregator.trim()
				? evt.aggregator.trim()
				: "aggregator";
	...
	function handleSessionCommandEvent(evt: RunEvent) {
		...
		if ((evt as any).terminal === true) {
			...
			msgs.forEach((m, i) => {
				...
				if (m.role === "tool" && m.toolStatus === "running") {
					msgs[i] = {
						...m,
						toolStatus: (evt as any).ok === false ? "error" : "done",
					};
				}
			});
		}
		...
		if (action === "branch" && (evt as any).ok !== false) {
			const branch = ((evt as any).branchSession || {}) as Record<
				string,
				unknown
			>;
			const newSessionId = String(
				(evt as any).newSessionId || branch.id || "",
			).trim();
			if (newSessionId) {
				...
				if (!existing) {
					sessions.value.unshift({
						id: newSessionId,
						profile:
							typeof branch.profile === "string" ? branch.profile : undefined,
						title: String(
							(evt as any).newSessionTitle || branch.title || "Branch",
						),
						...
						createdAt:
							typeof branch.createdAt === "number"
								...
								: Date.now(),
						updatedAt:
							typeof branch.updatedAt === "number"
								? branch.updatedAt
								: Date.now(),
						model: typeof branch.model === "string" ? branch.model : undefined,
						provider:
							typeof branch.provider === "string" ? branch.provider : undefined,
						messageCount:
							typeof branch.messageCount === "number"
								? branch.messageCount
								: undefined,
						messageTotal:
							typeof branch.messageCount === "number"
						...
						parentSessionId:
							typeof branch.parentSessionId === "string"
								...
								: typeof (evt as any).parentSessionId === "string"
									? (evt as any).parentSessionId
									: sid,
						forkPointMessageId:
							branch.forkPointMessageId != null
								? String(branch.forkPointMessageId)
								: null,
						parentTitle:
							typeof branch.parentTitle === "string"
								? branch.parentTitle
								: target?.title || null,
						parentLastMessage:
							typeof branch.parentLastMessage === "string"
								? branch.parentLastMessage
								: lastVisibleMessageContent(target?.messages),
						parentLastMessageRole:
							typeof branch.parentLastMessageRole === "string"
								? branch.parentLastMessageRole
								: lastVisibleMessageRole(target?.messages),
						workspace:
							typeof branch.workspace === "string" ? branch.workspace : null,
					});
				}
	...
	function handleAgentEvent(evt: RunEvent) {
		...
		if (!sid) return;
		if (
			(evt as any).source === "coding_agent" &&
			(evt as any).kind === "status"
		)
			return;
	...
	function applyGeneratedSessionTitle(evt: RunEvent) {
		const sid = evt.session_id;
		const title =
			typeof (evt as any).title === "string" ? (evt as any).title.trim() : "";
		if (!sid || !title) return;
		const target = sessions.value.find((s) => s.id === sid);
		if (target) {
			target.title = title;
	...
	function applySessionWorkspaceUpdate(evt: RunEvent) {
		const sid = evt.session_id;
		const workspace =
			typeof evt.workspace === "string" ? evt.workspace.trim() : "";
	...
	function completionNotificationAgent(session: Session): { icon: string } {
		const codingAgentId =
			session.codingAgentId || agentToCodingAgentId(session.agent);
	...
	function completionNotificationBody(
		session: Session,
		message?: Message,
	): string {
	...
	function showCompletionNotificationIfEnabled(
		sessionId: string,
	...
	async function sendMessage(content: string, attachments?: Attachment[]) {
		...
		const isBridgeSlashCommand =
			!isCodingAgentSession && isKnownBridgeSessionCommand(trimmedContent);
		const isBridgeCompressCommand =
			isBridgeSlashCommand && /^\/compress(?:\s|$)/i.test(trimmedContent);
		const isBridgePlanCommand =
			isBridgeSlashCommand && /^\/plan(?:\s|$)/i.test(trimmedContent);
		const isBridgeSkillCommand =
			isBridgeSlashCommand && /^\/skill(?:\s|$)/i.test(trimmedContent);
		const isBridgeBundleCommand =
			isBridgeSlashCommand && /^\/bundles(?:\s|$)/i.test(trimmedContent);
		const isBridgeMoaCommand =
			isBridgeSlashCommand && /^\/moa(?:\s|$)/i.test(trimmedContent);
		const isBridgeGoalCommand =
			isBridgeSlashCommand && /^\/goal(?:\s|$)/i.test(trimmedContent);
		const isBridgeForkCommand =
			isBridgeSlashCommand && /^\/fork(?:\s|$)/i.test(trimmedContent);
		const messageReference = isBridgeSlashCommand
			? null
			: messageReferences.value.get(sid) || null;
		const submittedContent = messageReference
			? formatMessageWithReference(messageReference, trimmedContent)
			: trimmedContent;
		const shouldOptimisticallyShowRunStatus =
			!isCodingAgentSession && !isBridgeForkCommand;
		...
		try {
			...
			if (attachments && attachments.length > 0) {
				...
				input = await buildContentBlocks(
					submittedContent,
					attachments,
					uploaded,
				);
				if (attachments.some((attachment) => attachment.context?.trim())) {
					displayInput = await buildContentBlocks(
						submittedContent,
						attachments,
						uploaded,
						false,
			...
			const profileModelGroups = sessionProfile
				? appStore.profileModelGroups.find(
						(entry) => entry.profile === sessionProfile,
					)?.groups
				: undefined;
			const runModelGroups = profileModelGroups?.length
				? profileModelGroups
				: appStore.modelGroups;
			const providerGroup = runModelGroups.find(
				(group) => group.provider === sessionProvider,
			);
			const storedSource = activeSession.value?.source;
			const sessionSource: StartRunRequest["source"] =
				storedSource === "global_agent"
					? "global_agent"
					: storedSource === "workflow"
						...
						: isCodingAgentSession
							...
							: storedSource === "api_server"
								? "api_server"
								: "cli";
			const isCodingAgentExecution =
				sessionSource === "coding_agent" ||
				(sessionSource === "workflow" && isCodingAgentSession);
			...
			const codingAgentApiMode =
				isCodingAgentExecution && codingAgentMode !== "global"
					? normalizeCodingAgentApiMode(
			...
			const runPayload: StartRunRequest = {
				...
				model: isCodingAgentExecution
					...
					: shouldSendInitialSessionConfig
						? sessionModel || undefined
						: undefined,
				...
				...(runtimeMode.value === "global_agent"
					...
					: {}),
				...(sessionSource === "workflow"
					? { session_source: "workflow" as const }
					: {}),
				...(isCodingAgentExecution
					? {
							...
							baseUrl:
								codingAgentMode === "global"
									? undefined
									: activeSession.value?.baseUrl ||
										providerGroup?.base_url ||
										undefined,
							apiKey:
								codingAgentMode === "global"
									? undefined
									: activeSession.value?.apiKey ||
										providerGroup?.api_key ||
										undefined,
			...
			const applyReconnectResume = (data: ResumeSessionPayload) => {
				...
				if (Array.isArray(data.messages)) {
					...
					const previousReasoningAssistantMessageId =
						reasoningAssistantMessageId;
					const replayRunMarker =
						getReplayRunMarker(data.events) ?? activeRunMarker;
					target.messages = mapHermesMessages(data.messages as any[]);
					restorePersistedSubagentStreams(sid);
					target.loadedMessageCount =
						data.messageLoadedCount ?? data.messages.length;
					...
					const resumedReasoningAssistant =
						resumedAssistantState.reasoningAssistant;
					...
					if (resumedReasoningAssistant) {
						...
						if (resumedReasoningAssistant.reasoning)
							noteReasoningStart(resumedReasoningAssistant.id);
					} else {
						reasoningAssistantMessageId = null;
					}
				}
				...
				(evt: RunEvent) => {
					...
					switch (evt.event) {
						...
						case "thinking.delta": {
							...
							runProducedAssistantText = true;
							const msgs = getSessionMsgs(sid);
							const reasoningTargetId =
								reasoningAssistantMessageId || activeAssistantMessageId;
						...
						case "tool.failed": {
							...
							const toolName =
								evt.tool || evt.name || toolMsgs[toolMsgs.length - 1]?.toolName;
							if (isBackgroundDelegateToolPayload(toolName, output)) {
						...
						case "workspace.diff.completed": {
							activeAssistantMessageId = handleWorkspaceRunChangeEvent(
								sid,
								evt,
						...
						case "run.completed": {
							...
							if ((evt as any).parsed_content !== undefined) {
								...
								if (lastAssistant) {
									...
									addMessage(sid, {
										...
										content: parsedContent,
										reasoning:
											typeof (evt as any).parsed_reasoning === "string"
												? (evt as any).parsed_reasoning
												: undefined,
										timestamp: Date.now(),
									});
									finalOutputTrimmed = parsedContentTrimmed;
							...
							if (swallowedError) {
								...
								showCompletionNotificationIfEnabled(
									sid,
									completedAssistantMessageId,
								);
							}
							...
							handleTerminalWorkspaceRunChange(
								sid,
								evt,
	...
	function resumeServerWorkingRun(sid: string, force = false, passive = false) {
		...
		const ensureAbortHandle = () => {
			...
			streamStates.value.set(sid, {
				abort: () => {
					getChatRunSocket(runtimeTransport())?.emit("abort", {
						session_id: sid,
					});
				},
			});
		...
		const initializeResumedAssistantState = () => {
			const resumedAssistantState = resolveResumedAssistantState(
				getSessionMsgs(sid),
				{ activeRunMarker },
			);
			activeRunMarker = resumedAssistantState.runMarker;
			...
			if (resumedAssistantState.reasoningAssistant) {
				reasoningAssistantMessageId =
					resumedAssistantState.reasoningAssistant.id;
	...
	function handlePeerUserMessage(evt: RunEvent) {
		...
		if (
			peer?.queued ||
			(!isPeerCommand && !wasDequeued && isSessionLive(sid))
		) {
