#!/usr/bin/env node
// cli: this is a CLI entry file — console I/O is by design
...
function shouldPreserveBridgeOnShutdown() {
	const raw = String(process.env.HERMES_AGENT_BRIDGE_STOP_ON_SHUTDOWN || "")
		.trim()
		.toLowerCase();
...
function getDaemonStopGraceMs(options = {}) {
	...
	if (restart && shouldPreserveBridgeOnShutdown()) {
		return (
			envPositiveInt("HERMES_WEB_UI_RESTART_GRACE_MS") ??
...
function getUpdatePort() {
	...
	if (process.env.PORT && !Number.isNaN(process.env.PORT))
...
function parseUnixNetstatListeningPids(out, port) {
	...
	for (const line of out.split(/\r?\n/)) {
		...
		const state = parts.find(
			(part) =>
				part.toUpperCase() === "LISTEN" || part.toUpperCase() === "LISTENING",
		);
...
function getListeningPids(port) {
	...
	if (commandExists("netstat")) {
		try {
			const out = execFileSync("netstat", ["-anp", "tcp"], {
				encoding: "utf-8",
				stdio: ["ignore", "pipe", "ignore"],
			});
...
function killListeningPids(port, pids = getListeningPids(port)) {
	...
	console.log(
		`  ⚠ Port ${port} is in use by PID(s): ${pids.join(" ")}, killing...`,
	);
	try {
		if (process.platform === "win32") {
...
function stopPreviewRuntimeFromCli() {
	...
	const pids = [
		...new Set(previewPorts.flatMap((port) => getListeningPids(port))),
	];
...
function startDaemon(port) {
	...
	if (existing && isRunning(existing)) {
		console.log(`  ✗ hermes-studio is already running (PID: ${existing})`);
		console.log(`    Use "hermes-studio stop" to stop it first`);
		process.exit(1);
	}
	removePid();
	...
	try {
		...
		if (stat.size > MAX_LOG_SIZE) {
			...
			console.log(
				`  ↻ Log rotated (${(stat.size / 1024 / 1024).toFixed(1)}MB → ${kept.length} lines)`,
			);
		}
	} catch {}
	...
	console.log(
		`  ⏳ Starting hermes-studio (PID: ${child.pid}, port: ${port})...`,
	);

	function poll() {
		waited += interval;
		if (!isRunning(child.pid)) {
			console.log("  ✗ Failed to start hermes-studio");
		...
		fetch(healthUrl)
			.then((res) => {
				if (res.ok) {
					...
					console.log(`  ✓ hermes-studio started`);
					console.log(`    ${url}`);
					...
					console.log(
						`  ⚠ Server process is running but health check failed after ${maxWait / 1000}s`,
					);
					console.log(`    Check log: ${LOG_FILE}`);
					const url = `http://localhost:${port}`;
					console.log(`    ${url}`);
...
function stopDaemon(options = {}) {
	...
	if (pidFromFile && !isRunning(pidFromFile)) {
		removePid();
		console.log(
			`  ✓ hermes-studio was not running (cleaned stale PID: ${pidFromFile})`,
		);
		pidFromFile = null;
	...
	if (!pid) {
		...
		if (stoppedPreviewPids) {
			console.log(`  ✓ hermes-studio preview stopped`);
			return;
		}
		console.log("  ✗ hermes-studio is not running");
		process.exit(1);
	}

	if (!isRunning(pid)) {
		removePid();
		console.log(`  ✓ hermes-studio was not running (cleaned stale PID)`);
		return;
	}
	...
	try {
		...
		removePid();
		console.log(`  ✓ hermes-studio stopped (PID: ${pid})`);
	} catch (err) {
...
function showStatus() {
	...
	if (pid && isRunning(pid)) {
		console.log(`  ✓ hermes-studio is running (PID: ${pid})`);
...
function clearLoginLocks(options = {}) {
	...
	const serverRunning = checkRunning ? Boolean(getPid()) : false;
	let removed = false;
	...
	try {
		...
		if (err?.code === "ENOENT") {
			if (!silent)
				console.log(`  ✓ No login lock file found: ${LOGIN_LOCK_FILE}`);
		} else {
			if (!silent)
				console.log(`  ✗ Failed to remove login lock file: ${err.message}`);
			throw err;
		}
	}
	...
	if (!silent && serverRunning) {
		console.log(
			"  ⚠ hermes-studio is running; restart it to clear in-memory login locks.",
		);
		console.log("    Run: hermes-studio restart");
	}
...
async function main() {
	...
	if (["-v", "--version", "version"].includes(command)) {
		console.log(`hermes-studio v${VERSION}`);
		process.exit(0);
	}

	if (["-h", "--help", "help"].includes(command)) {
		console.log(`
hermes-studio v${VERSION}
	...
	switch (command) {
		...
		case "clear-login-locks": {
			...
			if (restartAfterClear && result.serverRunning) {
				stopDaemon({ restart: true });
				setTimeout(() => startDaemon(getRunningPort() ?? getPort()), 500);
			}
			break;
		}
